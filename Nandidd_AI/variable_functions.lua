...
function var_dominance()
    ...
    if (dominance_ratio >= 1.4 and dominance_diff >= 1000) then -- change this threshold depending on income (or maybe absolute fact_selfValue)
        dominance_flag = 1
    else 
        dominance_flag = 0
    end

    if (dominance_ratio <= 0.7 and dominance_diff <= -1000) then -- change this threshold depending on income (or maybe absolute fact_selfValue)
        defence_flag = 1
    else 
        defence_flag = 0
    end
...
function var_desired_henchmen()
    ...
    if (is_gather_site_open == 1) then
        ...
        return min(max_henchmen, 10*curRank) -- make more detailed logic
    end
...
function var_upgrades_worth()
    ...
    if num_soundbeamsQ + num_aatowersQ >= 5 then
